...
# from build import fitter

class analyzer:

    def analyze_fit(self,fit_info,q=deque()):
        # set up canvas
        ...
        with open(fit_info,'r') as json_file:
            info = json.load(json_file)
        file_name = info['file_name']
        print "USING MODEL",fit_info
        print "USING DATA FROM",file_name
        ftr = fitter(file_name,fit_info=fit_info,fitted=True)
        ...
        while not cmd == "" and\
            ...
            if cmd == 's':
                ...
                json_name = "../out/fits/%sfitter-%s-%s-%s.json" % (keyword,ftr.fit_name,ftr.pname,
                ...
                jpg_name = "../out/plots/%sfitter-%s-%s-%s.jpg" % (keyword,ftr.fit_name,ftr.pname,
                        ftr.file_name[eta_start:eta_start + num_start] + "%04i"%(num))
                if len(q) > 0:
                    affirm2 = q.pop()
                else:
                ...
                if not hasStats:
                    ...
                    canv.Update()
                    if drawfunc or drawfhist:
                        st = self.__stats(st,"FUNC",chi=ftr.chi,func=func,NDF=ftr.NDF)
                        st.Draw()
                        canv.Update()
                else:
                    hasStats = False
    ...
    def __legend(self,lgn=None,option="ON",**kwargs):
        if option == "ON":
    ...
    def __stats(self,st=None,option="ON",**kwargs):
        if option == "ON":
