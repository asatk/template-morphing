"""
Train a regression DCGAN with n>=2 regression variables
...
def train_CcGAN(train_samples, train_labels, netG, netD, save_images_folder, save_models_folder = None, plot_in_train=False, fig_size=5, point_size=None):
    ...
    for niter in range(resume_niters, niters):
        ...
        batch_epsilons = np.ndarray((nvars,batch_size_disc))
        for v in nvars:
            batch_epsilons_v = np.random.normal(0,sigmas[v],batch_size_disc)
            batch_epsilons[v] = batch_epsilons_v
        ...
        batch_real_indx = np.zeros((nvars,batch_size_disc), dtype=int) #index of images in the datata; the labels of these images are in the vicinity
        batch_fake_labels = np.zeros((nvars,batch_size_disc))
