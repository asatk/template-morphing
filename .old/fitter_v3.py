...
class fitter:
    ...
    def __init_new(self):
        ...
        self.seed = info['seed'] if info['seed'] != "" else self.fit_name
        self.debug = int(info['debug'])
        self.window_time = int(info['window_time'])
        self.thresh = float(info['thresh'])
    ...
    # need to exclude bins of less than cutoff/alpha = 0.02? can vary porportion as needed
    ...
    def fit(self):
        ...
        self.nEntries = self.hist.Integral()
        ...
        if self.seed == 'file':
            ...
            seed_file_str = PROJECT_DIR+"/out/fits/%s/%s/%s_fitter-%s-%s-%s-%s.json"
            eta_start = self.file_name.find('eta')
            num_match = re.search("\\d+(?=\\.root)",self.file_name[eta_start:])
            num = int(num_match.group())
            num_start = int(num_match.start())
            run_date = self.file_name[self.file_name.find('_')+1:self.file_name.rfind('_')]
            seed_file = seed_file_str%(
                    self.pname,self.fit_name,
                    run_date,
            ...
            # fill fn params with those suggested in previous fit
        ...
        if self.seed == 'gaus':
            ...
            fn.SetParLimits(2, 0., mean_est)
        elif self.seed == 'crystalball':
            fn.SetParName(0, 'Constant')
            fn.SetParName(1, 'Mean')
            fn.SetParName(2, 'Sigma')
            ...
            #fn.SetParLimits(0, 0., self.nEntries)
            fn.SetParLimits(1, 0., mean_est * 2)
            fn.SetParLimits(3, 0., 1e4)
            fn.SetParLimits(4, 0., 1e4)
        elif fit_name == 'landau':
            fn.SetParName(0, 'Constant')
            fn.SetParName(1, 'MPV')
            fn.SetParName(2, 'Eta')
            fn.SetParameter('Constant', random.uniform(
        ...
        if debug == 0:
            ...
            fit_opts += "V"
        fit_res = self.hist.Fit("fit", fit_opts)
        fit_fn = self.hist.GetFunction("fit")
    ...
    # create histogram with basic parameters
    def __setup_hist(self):
        if self.debug >= 1:
            print "TESTING %s FIT\n" % (self.fit_name)
        hist = ROOT.TH1D("hist", "plot %s fit" %
                         (self.fit_name), self.bins, self.lo, self.hi)
    ...
    def __adjust_hist(self):
        ...
        maxval = self.hist.GetBinContent(maxbin)
        maxX = self.hist.GetXaxis().GetBinCenter(maxbin)

        print "max bin, max value:",maxbin,maxval

        # cut out small bins
        for bin in range(self.hist.GetNbinsX()):
            binval = self.hist.GetBinContent(bin)
            binX = self.hist.GetXaxis().GetBinCenter(bin)
            if abs(binX-maxX) > self.width or self.hist.GetBinContent(bin) < self.thresh*maxval:
                self.hist.SetBinContent(bin,0.005)
    ...
    def jsonify(self,fit_info=""):
        if fit_info == "":
            ...
            # FILE NAMING CONVENTION!!! WILL CHANGE IF MORE IS ADDED TO ROOT FILE NAMES
            date_start = self.file_name.find('_') + 1
            date_end = self.file_name.rfind('_')
            date = self.file_name[date_start:date_end]
            ...
            fit_info = fit_name_dir+"/%s_fitter-%s-%s-%s-%s.json" % (
                    date, "norm" if self.normalized else "",self.fit_name,self.pname,
                    self.file_name[eta_start:eta_start + num_start] + "%04i"%(num))
        ...
        info['file_name'] = re.sub(PROJECT_DIR,r"${PROJECT_DIR}",self.file_name)
