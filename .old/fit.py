...
fits = {
    'gaus':'gaus',
    'gaus_pdf':'[2]*ROOT::Math::normal_pdf(x[0],[0],[1])'
    'gaus_cdf':'[2]*ROOT::Math::normal_cdf(x[0],[0],[1])',
    'dbl_gaus':'gaus(0)+gaus(3)',
    'dbl_gaus_cdf':,
    'crys_ball':'ROOT::Math::crystalball_function(x,[0],[1],[2],[3])',
    'crys_ball_pdf':'[4]*ROOT::Math::crystalball_pdf(x,[0],[1],[2],[3])'
    'crys_ball_cdf':'[4]*ROOT::Math::crystalball_cdf(x,[0],[1],[2],[3])',
    'landau':'landau',
    'landau_cdf':,
    'landxgaus':,
    'landxgaus_cdf':
...
def test_fit(fit_name="gaus"):
    ...
    if fit_name in ['gaus','gaus_pdf','gaus_cdf']
        ...
        fn.SetParName(1,'Sigma')
        fn.SetParName(2,'Mean')
    ...
    if user_data:
        ...
        hist2.FillRandom(fit_str,nfill)
        hist2.Draw()
    if scale:
    ...
    fit_gaus_res = hist2.Fit("gaus_fn","SM0")
    ...
    print "TEST %s - END"%s(fit_name)


#Pad 1 - simplefn Fill + Fit
if start <= 1 and end >= 1:
    
    print "TEST #1 - SIMPLE FN"

    canv.cd(1-start+1)
    hist1 = ROOT.TH1D("hist1","plot simplefn",bins,lo,hi)

    #fn = ROOT.TF1("simplefn","-1*[0]*sin(x+20)*sqrt(x+20)",lo,hi)
    #fn = ROOT.TF1("simplefn","[0]*sqrt(x+20)",lo,hi)
    fn = ROOT.TF1("simplefn","[0]*(x-[1])**2",lo,hi)
    fn.SetParameters(-100,500)
    fn.SetLineColor(46)
    #fn.Draw("same")

    if user_data:
        draw_s = info['var'] + ">>hist1"
        cut_s = info['cuts']
        chain.Draw(draw_s,cut_s)
    else:
        hist1.FillRandom("simplefn",nfill)
        hist1.Draw()
    if scale:
        hist1.Scale(1/hist1.Integral())

    hist1.SetFillColor(35)

    hist1.Fit("simplefn","SL")
    #fit_simple = hist1.GetFunction("simplefn")
    #fit_simple.Draw("same")
...
cmd = " "
while not cmd == "" and\
    not cmd == "/home/asatk/miniconda3/envs/cern2.7/bin/python /home/asatk/Documents/code/cern/TM/fit.py":
    time.sleep(3)
    cmd = raw_input()