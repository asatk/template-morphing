...
class fitter:
    ...
    def get_fit(self):
        ...
        if self.normalize:
            ...
            print "at mean",fit_fn.Eval(fit_fn.GetParameter('x0/Mean'))
            print self.lo,self.hi
            if debug >= 2:
                print "NORMALIZING HIST BY FACTOR OF %f"%(1./intg)
                print "intg",intg
                print "NORMALIZING FUNC BY FACTOR OF %f"%(norm_factor)
                print "intg",1./norm_factor
            fit_fn.SetParameter('Constant',fit_fn.GetParameter('Constant')*norm_factor)
            print "intg",fit_fn.Integral(self.lo,self.hi)
